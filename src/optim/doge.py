...
def get_model_grad_dict(model):
    ...
    return {p_name: p.grad for p_name, p in model.named_parameters()}


def add_model_grad_ls(model, domain_full_grad_dicts, dw):
...
class DoGE:
    ...
    def __call__(self, pertoken_losses, token_masks, domain_ids, current_lr):
        ...
        for domain_id, curr_domain_losses in enumerate(all_domain_losses):
            if curr_domain_losses is None:
                ...
                domain_flat_grad = get_model_grad_flat(self.model)
