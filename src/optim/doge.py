...
class DoGE:
    def __init__(self, model, num_experts, train_ids, tgt_ids, args, device="cuda"):
        self.model = model
        self.num_experts = num_experts
        self.device = device
        self.args = args

        self.train_ids = torch.tensor(train_ids, dtype=torch.long, device=device)
        self.tgt_ids = torch.tensor(tgt_ids, dtype=torch.long, device=device)
        ...
        # domain weights
        self.train_dw = torch.ones(num_experts, device=device) / len(train_ids)
        self.avg_dw = torch.zeros(num_experts, device=device)

        # accumulation buffers
        self.flat_grad_mat = torch.zeros((num_experts, self._num_params()), device=device)
        self.tgt_accumulation = torch.zeros_like(self.flat_grad_mat)
        self.iter_domain_losses = torch.zeros(num_experts, device=device)
    ...
    def _num_params(self):
        """Count total params to flatten grads into one vector."""
        ...
        return num_activated_params

    def _accumulate_domain_grads(self, pertoken_losses, domain_ids, accumulator, record_losses=True):
        for domain_id in range(self.num_experts):
            mask = (domain_ids == domain_id)
            if mask.sum() == 0:
                continue

            loss = pertoken_losses[mask].mean()
            if record_losses:
                self.iter_domain_losses[domain_id] = loss.item()
            ...
            flat_grad = torch.cat([g.reshape(-1) for g in grads if g is not None and torch.norm(g) > 0])
            accumulator[domain_id][:] += flat_grad.detach()

            del grads, flat_grad, loss

    def __call__(self, pertoken_losses, domain_ids):
        self._accumulate_domain_grads(pertoken_losses, domain_ids, self.flat_grad_mat)
        self.accumulation_steps += 1

    def ood_call(self, tgt_losses=None, tgt_domain_ids=None):
        """
        If tgt_losses provided → accumulate gradients from new (OOD) dataset.
        Otherwise → just clone from train grads (self-target setup).
        """
        if tgt_losses is not None and tgt_domain_ids is not None:
            # true OOD accumulation
            self._accumulate_domain_grads(tgt_losses, tgt_domain_ids, self.tgt_accumulation, record_losses=False)
        else:
            # in-domain reweighting (copy)
            self.tgt_accumulation = self.flat_grad_mat.clone()

    def reweight(self, current_lr):
        wandb_log_dict = {}

        if self.accumulation_steps == 0:
            return wandb_log_dict  # nothing accumulated

        # mean target grads (self-target here)
        avg_tgt_grads = self.tgt_accumulation[self.tgt_ids] / self.accumulation_steps
        train_grads = self.flat_grad_mat[self.train_ids] / self.accumulation_steps
        ...
        for i, domain_idx in enumerate(self.train_ids.tolist()):
            ...
            wandb_log_dict[f"avg_dw/{domain_name}"] = (self.avg_dw[domain_idx] / self.dw_update_steps).item()
...
#                 del grads, model_flat_grad, domain_mask, curr_domain_loss
